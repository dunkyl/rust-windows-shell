...
macro_rules! registry_items {
    ...
    // Set value named by format expression
    ($name:literal=$val:tt $($rest:tt)*) => {
        vec_prefixed!{
            registry::RegOp::Value(
                Some(format!($name).into()), $val.into()),
            registry_items!($($rest)*)
        }
    };
    // Set value named by variable, or parentesized expression
...
macro_rules! registry {
    ($rootKey:ident, $($items:tt)+) => {
        registry::RegBatch {
            key: $rootKey,
            operations: registry_items!($($items)+)
        }
    };
}